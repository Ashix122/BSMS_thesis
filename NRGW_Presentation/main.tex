...
\title[Critical Phenomenon in Axisymmetric Spacetimes]{Critical Phenomenon in Axisymmetric Vacuum Spacetimes: Role of Twist}
\subtitle{\Large Phase 1 Presentation}
\author[Ashique T Nizar]{\textbf{Ashique T Nizar}}
\institute[]{\Large Indian Institute of Science Education and Research, Trivandrum \\[4pt] \large Under the guidance of Dr.\ Soumen Basak and Dr.David Hilditch}
...
$$
where t is the time and $x^i$ is the spatial coordinates
$$
R_{\mu \nu}-\frac{1}{2}g_{\mu \nu}R =8 \pi T_{\mu \nu}
$$
...
\item As we deviate from spherical symmetry, we began to see a ``loss" in critical phenomenon. In the similiar massless scalar field test done in axisymmetry, they found 
...
\begin{figure}
    \centering
    \includegraphics[width=0.3\linewidth]{deviation.png}
    \caption{Deviations observed (Choptuik et al, 2003)}
    \label{fig:placeholder}
\end{figure}
\end{column}
\begin{column}{0.42\linewidth}
    Here $\epsilon$ marks the inital-data's deviation from spherical symmetry. Here as we can see that eventhough everyone of the families are axisymmetric,the \emph{universality} of the critical exponent is gone now
\end{column}
\end{columns}
\end{frame}
...
But on the introduction of angular momentum into the initial data, we begin to recover perfect critical phenomenon as before 
\begin{columns}
...
\begin{alertblock}{}
    \begin{itemize}
        \item Axisymmetry is characterised by the presence of a spacelike Killing vector field and twist measures its vorticity
         \item Angular momentum implies twist but twist does not necessarily imply the presence of angular momentum
    \end{itemize}
\end{alertblock}
\end{column}
\end{columns}
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{separate_scaling_new.pdf}
\end{figure}
...
\begin{frame}{How do we tune and why is it hard?}
\vspace{100pt}
To observe supercriticality, we probe for the formation of \textbf{Apparent horizons} (AHs) in the slice. Singularity theorems guarantee a singular spacetime if AHs are formed in the spacetime.
\end{frame}
...
where D is the induced 3-metric, $K_{ij}$ the extrinsic curvature, $s^i$ the normal vector to the surface. We find surfaces that satisfies this condition.

\end{column}
\begin{column}{0.42\linewidth}
    \begin{figure}
        \centering
        \includegraphics[width=1\linewidth]{figure.pdf}
...
\begin{alertblock}{Cartesian based finder}
    Previous studies in vacuum axisymmetry didnt necessitate the usage of a different parameterization since it very much simplifies the finding problem, and they were able to tune high values of precision.
\end{alertblock}
...
\begin{itemize}
    \item Using the new AH finder, we were able to find disjoint AHs around z axis values where the Kretschmann scalar is the maximum in the slice
...
\begin{itemize}
    \item Hence we expect that breakdown of the parameterisation was the real cause behind why we couldn't classify 
    \item With this new AH finder we will be able to tune and reach the critical regime in a much faster rate from now on
...
\begin{frame}{Conclusion and future directions}
    \begin{itemize}
    \item We currently set up the framework to create and evolve twisting brill wave initial data for arbitrary families and to characterise the critical phenomenon
        \item With the initial family, we have tuned upto 3 decimal places and solved some of the issues that we face while tuning towards the threshold. Further tuning is being performed with improvements we made to the tuning process.

        \item After sufficient tuning, we will begin our analysis of the critical threshold of twisting vacuum spacetimes. (Critical exponents,  self similarity and echoing constants.) 
        \item Construct new families (Eg.Impure twisting) and do the similiar exercise and compare the phenomenon between families and check for universality
        \item Devise strategies to make the new AH finding process automative 
    \end{itemize}
\end{frame}
